...
const Profile = () => {
  const { userId } = useParams(); // Get the dynamic user ID from URL
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        if (!userId || userId === currentUser.uid) {
          ...
          setUser({ uid: userId }); // Load other user's profile
          setIsCurrentUser(false);
        }

        try {
          const userExtraInfo = await getExtraInfo(userId || currentUser.uid);
          if (userExtraInfo) {
